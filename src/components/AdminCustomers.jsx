...
function AdminCustomers() {
  ...
  const fetchCustomers = async () => {
    try {
      ...
      const response = await fetch(
        'http://localhost:5001/api/orders/admin/customers'
      )
      ...
      setError(
        'Unable to load customers. Please check that the backend is running.'
      )
    } finally {
      setIsLoading(false)
    }
  ...
  const fetchCustomerOrders = async (customer) => {
    try {
      ...
      const response = await fetch(
        'http://localhost:5001/api/orders/admin/all'
      )
      ...
      if (!response.ok) {
        throw new Error(
          data.message || 'Failed to load customer orders'
        )
      }
